...
export class PdfService {
  ...
  private static async generateTermSpecificBulkReportWithJsPDF(students: Student[], selectedTerm: number): Promise<Blob | null> {
    try {
      ...
      doc.setFontSize(8)
      doc.text(`Generated: ${timestamp}`, 15, 270)
      doc.text(`Term ${selectedTerm} Performance Report - ${students.length} Students`, 15, 276)
      ...
      if (students.length > 0) {
        ...
        const maxRows = Math.min(students.length, 15)
        
        students.slice(0, maxRows).forEach((student, index) => {
          ...
          doc.text(name, colPositions[0], rowY + 5)
          doc.text(school, colPositions[1], rowY + 5)
          doc.text(`${student.average}%`, colPositions[2], rowY + 5)
          doc.text(student.status, colPositions[3], rowY + 5)
        })

        if (students.length > maxRows) {
          yPos += (maxRows * 8) + 10
          doc.setTextColor(107, 114, 128)
          doc.setFontSize(8)
          doc.text(`... and ${students.length - maxRows} more students`, 20, yPos)
        }
      }
      ...
      doc.setTextColor(107, 114, 128)
      doc.setFontSize(8)
      doc.text('Sol Plaatje University - Talent Pipeline Programme - Term Analytics Report', 105, 285, { align: 'center' })
