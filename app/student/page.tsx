...
export default function StudentDashboard() {
      ...
      try {
        ...
        if (user) {
          ...
          if (profile.grade) {
            ...
            toast.success(`Grade ${profile.grade} automatically selected from your profile`, {
              position: 'top-right',
              duration: 3000,
              icon: <CheckCircle className="h-4 w-4" />,
            })
          } else {
            toast.info(`Please complete your profile with your current grade for automatic selection`, {
              position: 'top-right',
              duration: 4000,
          ...
          if (profile.school) {
            ...
            if (matchingSchool) {
              ...
              toast.success(`${matchingSchool.label} automatically selected from your profile`, {
                position: 'top-right',
                duration: 3000,
            ...
            toast.info(`Please complete your profile with your school information for automatic selection`, {
              position: 'top-right',
              duration: 4000,
              icon: <Edit3 className="h-4 w-4" />,
            })
          ...
          toast.success(`Welcome back, ${profile.first_name}! Your profile data has been loaded.`, {
            position: 'top-right',
            duration: 4000,
          })
    ...
    const loadTermData = async () => {
      try {
        ...
        if (termData.completed) {
          toast.success(`Term ${selectedTerm} data loaded successfully`, {
            position: 'top-right',
            duration: 3000,
          })
        } else {
          ...
          toast.info(`Ready to input Term ${selectedTerm} data`, {
            position: 'top-right',
            duration: 2000,
          })
        }
        ...
        toast.error('Failed to load term data. Please try again.', {
          position: 'top-right',
          duration: 5000,
        })
      } finally {
  ...
  const calculateResults = async () => {
    try {
      ...
      if (currentTermSubjects.length === 0) {
        ...
        toast.error('Please add at least one subject for this term')
      ...
      if (invalidSubjects.length > 0) {
        ...
        toast.error('Please ensure all percentages are between 0 and 100')
        return { success: false }
      }
      ...
      toast.success(`Term ${selectedTerm} results saved successfully! Grade ${grade} and school info updated in your profile.`, {
        position: 'top-right',
        duration: 5000,
      ...
      toast.error('Failed to calculate and save performance summary: ' + (error instanceof Error ? error.message : 'Unknown error'))
      return { success: false }
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (incompleteSubjects.length > 0) {
      toast.error(`Please complete all fields (Level, Final %, Grade Average %) for all subjects in Term ${selectedTerm}`, {
        position: 'top-right',
        duration: 6000,
        icon: <XCircle className="h-4 w-4" />,
      })
    ...
    toast.info(`Saving Term ${selectedTerm} data...`, {
      position: 'top-right',
      duration: 2000,
    })
    ...
    try {
      ...
      setIsSubmitted(true)
      toast.success(`Term ${selectedTerm} data saved successfully! Average: ${Math.round(calculatedAverage)}%`, {
        position: 'top-right',
        duration: 5000,
        icon: <PartyPopper className="h-4 w-4" />,
      })
